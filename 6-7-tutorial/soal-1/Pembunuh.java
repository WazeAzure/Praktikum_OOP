 ...
 */
 ...
 public class Pembunuh implements Kelas {
    ...
    public void serang(Karakter self, Karakter target) {
        System.out.println(self.getName() + ", sang pembunuh, menyerang " + target.getName() + " dengan " + getSenjata());
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()) {
            ...
            if (target.getHealth() < 20) {
                serap = target.getHealth();
            } else {
                serap = 20;
            }
            System.out.println(self.getName() + " menyerap health " + target.getName() + " sebesar " + serap);
            target.terimaSerangan(serap);
            self.addHealth(serap);
        } else {
