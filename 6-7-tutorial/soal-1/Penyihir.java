 ...
 */
 ...
 public class Penyihir implements Kelas {
    ...
    public void serang(Karakter self, Karakter target) {
        if (self.getMana() >= 20) {
            System.out.println(self.getName() + ", merapal " + getSihir() + " kepada " + target.getName());
            self.reduceMana(20);
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()) {
            ...
            if (target.getMana() < 40) {
                serap = target.getMana();
            } else {
                serap = 40;
            }
            System.out.println(self.getName() + " menyerap " + serap + " mana dari " + target.getName());
            target.reduceMana(serap);
            self.addMana(serap);
